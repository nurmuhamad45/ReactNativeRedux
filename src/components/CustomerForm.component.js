...
class CustomerForm extends Component {
  ...
  renderTextfield(options) {
    return <TextInput style={styles.textField} onChangeText={(value) => this.setState({ [options.name]: value })} placeholder={options.label} value={this.state[options.name]} keyboardType={options.keyboard || "default"} />;
  }
  ...
  render() {
    return (
      <View style={styles.panel}>
        {this.renderTextfield({ name: "name", label: "Nama" })}
        {this.renderTextfield({ name: "phone", label: "No. HP", keyboard: "phone-pad" })}
